...
#          reference page for the Careproctus argosgeorgiae poster (2026).
#
...
url <- "https://martinglhf.github.io/careproctus-argosgeorgiae-references-poster/"
cat("Generating QR code for URL:\n", url, "\n\n")

# Encode QR as matrix --------------------------------------------------------

qr <- qrencode_raw(url)   # raw matrix version (TRUE/FALSE)

# Preview (black & white) ----------------------------------------------------

par(mar = c(0, 0, 0, 0))
image(t(qr[nrow(qr):1, ]),
      col = c("white", "black"),
      axes = FALSE,
      xlab = "",
      ylab = "")
